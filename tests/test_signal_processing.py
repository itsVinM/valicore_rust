...
class TestRustSignalProcessor:
    ...
    def test_thd(self, sine_wave):
        samples, sample_rate = sine_wave
        thd_val = RustSignalProcessor.thd(samples, 50.0, sample_rate)
        assert thd_val < 5.0

    def test_window_hann(self, sine_wave):
        samples, _ = sine_wave
    ...
    def test_window_hamming(self, sine_wave):
        samples, _ = sine_wave
        windowed = RustSignalProcessor.window(samples, "hamming")
        assert len(windowed) == len(samples)

    def test_window_blackman(self, sine_wave):
        samples, _ = sine_wave
        windowed = RustSignalProcessor.window(samples, "blackman")
        assert len(windowed) == len(samples)

    def test_window_invalid(self, sine_wave):
        samples, _ = sine_wave
